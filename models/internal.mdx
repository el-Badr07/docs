...
Comprehensive model for managing Celery background tasks with detailed tracking and monitoring.
...
- **task_type**: Category of task (invoice_processing, export_generation, system_maintenance)
- **priority**: Task priority level (low, normal, high, critical)

**Execution Context**:
- **queue_name**: Celery queue where the task is executed
...
Manages task queue statistics and performance monitoring.
