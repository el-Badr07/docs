---
title: "Task Manager (RabbitMQ)"
description: "High-throughput background task processing with RabbitMQ and async workers"
---
The RabbitMQ Task Manager forms the backbone of Textra's asynchronous processing capabilities, enabling scalable, reliable, and efficient background processing of accounting workflows without the Celery runtime.

## Task Architecture

### Distributed Task Processing with RabbitMQ

The Task Manager is designed around the same core principles as before, but implemented with native async Python and RabbitMQ:

- **Scalability**: Simply start more `python -m src.message_worker.run_worker` instances
- **Reliability**: Robust error handling, retry, and dead-letter queues
- **Observability**: Real-time status cached in Redis; progress endpoints `/api/accounting/task-status/{task_id}`
- **Flexibility**: Multiple task types defined in `src/models/queue_models.py`
- **Performance**: Prefetch + QoS tuning in `queue_service.py`
...
The RabbitMQ Task Manager provides a robust, scalable, and reliable foundation for all background processing needs in Textra, ensuring that accounting workflows can be processed efficiently and reliably while maintaining the highest standards of performance, security, and compliance. 